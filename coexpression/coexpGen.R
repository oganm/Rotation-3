...
# get group means ----
selectGroupData = matrix(0, nrow = nrow(exprData), ncol = length(unique(design$ourNamingGeneral)))
groupNames = unique(design$ourNamingGeneral)
colnames(selectGroupData) = groupNames
for (i in 1:length(groupNames)){
    groupIndex = design$ourNamingGeneral == groupNames[i]
...
for ( i in 1:ncol(selectGroupData)){
    corCheck = which(geneData$Gene.Symbol %in% foldGenes[[which(names(foldGenes) == unique(design$ourNamingGeneral)[i])]])
    ...
    for (j in seq(0.9,0.1,-0.1)){
        ...
        cType= c(i, sample({1:ncol(selectGroupData)}[-i],4))
        cType={1:ncol(selectGroupData)}[-i]
        for ( z in 1:30){
            m = rnorm(1, j, sd = 0.05)
            if (m > 1){
                m = 1
            ...
            randomSemp = generateSamples(noiseMax = 0.1, randomNo = 1, mixSamp = c(i,cType) , props = c(m, rep((1-m)/15,15)))
    ...
    ggsave(filename ='111' + colnames(selectGroupData)[i]+' ' + unique(groups)[j] + '.png')
